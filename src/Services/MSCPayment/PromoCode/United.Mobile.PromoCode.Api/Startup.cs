...
namespace United.Mobile.PromoCode.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddTransient<IPromoCodeBusiness, PromoCodeBusiness>();
        ...
        public void ConfigureContainer(ContainerBuilder container)
        {
            ...
            container.Register(c => new ResilientClient(Configuration.GetSection("GetTermsandCondtionsClient").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("GetTermsandCondtionsClientKey");
            container.RegisterType<GetTermsandCondtionsService>().As<IGetTermsandCondtionsService>().WithAttributeFiltering();

            container.Register(c => new ResilientClient(Configuration.GetSection("GetCCEContentClient").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("GetCCEContentClientKey");
            container.RegisterType<GetCCEContentService>().As<IGetCCEContentService>().WithAttributeFiltering();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory, IApplicationEnricher applicationEnricher, IFeatureSettings featureSettings, IHostApplicationLifetime applicationLifetime)
        {
            ...
            app.MapWhen(context => string.IsNullOrEmpty(context.Request.Path) || string.Equals("/", context.Request.Path), appBuilder =>
            {
                ...
                {
                    await context.Response.WriteAsync("Welcome from PromoCode Microservice");
                });
            });
        ...
        private async Task OnStart(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.LoadFeatureSettings(ServiceNames.PROMOCODE.ToString());
            }
            catch (Exception) { }
        ...
        private async Task OnShutDown(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.DeleteContainerIPAdress(ServiceNames.PROMOCODE.ToString(), StaticDataLoader._ipAddress);
            }
            catch { }
        }
    }
