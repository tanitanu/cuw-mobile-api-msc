...
namespace United.Mobile.TravelCredit.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddTransient<ITravelCreditBusiness, TravelCreditBusiness>();
        ...
        public void ConfigureContainer(ContainerBuilder container)
        {
            ...
            container.Register(c => new ResilientClient(Configuration.GetSection("OTFConversionClient").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("OTFConversionClientKey");
            container.RegisterType<OTFConversionService>().As<IOTFConversionService>().WithAttributeFiltering();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory, IApplicationEnricher applicationEnricher, IFeatureSettings featureSettings, IHostApplicationLifetime applicationLifetime)
        {
            ...
            app.MapWhen(context => string.IsNullOrEmpty(context.Request.Path) || string.Equals("/", context.Request.Path), appBuilder =>
            {
                ...
                {
                    await context.Response.WriteAsync("Welcome from TravelCredit Microservice");
                });
            });
        ...
        private async Task OnStart(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.LoadFeatureSettings(ServiceNames.TRAVELCREDIT.ToString());
            }
            catch (Exception) { }
        ...
        private async Task OnShutDown(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.DeleteContainerIPAdress(ServiceNames.TRAVELCREDIT.ToString(), StaticDataLoader._ipAddress);
            }
            catch { }
        }
    }
