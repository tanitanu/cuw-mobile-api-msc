...
namespace United.Mobile.ETC.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddTransient<IETCBusiness, ETCBusiness>();
        ...
        public void ConfigureContainer(ContainerBuilder container)
        {
            ...
            container.Register(c => new ResilientClient(Configuration.GetSection("ETCBalanceClient").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("ETCBalanceClientKey");
            container.RegisterType<ETCBalanceEnquiryService>().As<IETCBalanceEnquiryService>().WithAttributeFiltering();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory, IApplicationEnricher applicationEnricher, IFeatureSettings featureSettings, IHostApplicationLifetime applicationLifetime)
        {
            ...
            app.MapWhen(context => string.IsNullOrEmpty(context.Request.Path) || string.Equals("/", context.Request.Path), appBuilder =>
            {
                ...
                {
                    await context.Response.WriteAsync("Welcome from ETC Microservice");
                });
            });
        ...
        private async Task OnStart(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.LoadFeatureSettings(ServiceNames.ETC.ToString());
            }
            catch (Exception) { }
        ...
        private async Task OnShutDown(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.DeleteContainerIPAdress(ServiceNames.ETC.ToString(), StaticDataLoader._ipAddress);
            }
            catch { }
        }
    }
