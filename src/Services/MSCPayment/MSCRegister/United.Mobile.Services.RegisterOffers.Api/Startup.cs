...
namespace United.Mobile.Services.RegisterOffers.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddScoped<IETCUtility, ETCUtility>();
            services.AddScoped<IMSCBaggageInfo, MSCBaggageInfo>();
            services.AddScoped<ICheckoutUtiliy, CheckoutUtiliy>();
            services.AddScoped<IShoppingUtility, ShoppingUtility>();
            services.AddScoped<IDataPowerFactory, DataPowerFactory>();
            services.AddScoped<IMSCFormsOfPayment, MSCFormsOfPayment>();
            services.AddHttpContextAccessor();
            services.AddScoped<IMSCRegisterBusiness, MSCRegisterBusiness>();
            services.AddScoped<IShoppingCartUtility, ShoppingCartUtility>();
            ...
            services.AddScoped<IMSCPkDispenserPublicKey, MSCPkDispenserPublicKey>();
            services.AddScoped<IMSCPageProductInfoHelper, MSCPageProductInfoHelper>();
        ...
        public void ConfigureContainer(ContainerBuilder container)
        {
            ...
            container.Register(c => new ResilientClient(Configuration.GetSection("memberProfileConfig").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("memberProfileConfigKey");
            container.RegisterType<MemberInformationService>().As<IMemberInformationService>().WithAttributeFiltering();
            ...
            container.Register(c => new ResilientClient(Configuration.GetSection("VerifyMileagePlusHashpinService").Get<ResilientClientOpitons>())).Keyed<IResilientClient>("VerifyMileagePlusHashpinServiceKey");
            container.RegisterType<VerifyMileagePlusHashpinService>().As<IVerifyMileagePlusHashpinService>().WithAttributeFiltering();
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IApplicationEnricher applicationEnricher, IFeatureSettings featureSettings, IHostApplicationLifetime applicationLifetime)
        {
            ...
            app.MapWhen(context => string.IsNullOrEmpty(context.Request.Path) || string.Equals("/", context.Request.Path), appBuilder =>
            {
                ...
                {
                    await context.Response.WriteAsync("Welcome from RegisterOffers Microservice");
                });
            });
        ...
        private async Task OnStart(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.LoadFeatureSettings(ServiceNames.MSCREGISTER.ToString());
            }
            catch (Exception) { }
        ...
        private async Task OnShutDown(IFeatureSettings featureSettings)
        {
            try
            {
                await featureSettings.DeleteContainerIPAdress(ServiceNames.MSCREGISTER.ToString(), StaticDataLoader._ipAddress);
            }
            catch { }
        }
    }
